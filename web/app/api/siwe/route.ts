...
export const POST = async (req: NextRequest) => {
	...
	try {
		...
		const { data: address, error } = await supabase.from('addresses').select('*').eq('address', fields.address).single();
		if (error) {
		...
		if (!address) {
			const { data: address, error } = await supabase.from('addresses').insert([{ address: fields.address }]).select().single();
			if (error) {
				return tap(new NextResponse(String(error), { status: 400 }), (res) =>
					session.clear(res)
				);
